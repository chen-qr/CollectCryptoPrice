...
address = Web3.to_checksum_address(uniswap_v3_config.get("address"))
assert web3_client.eth.get_balance(address) >= 0, "Balance is not enough!"

uniswap_contract = web3_client.eth.contract(address=address, abi=uniswap_v3_config.get("ABI"))

factoryV2Address = uniswap_contract.functions.factoryV2().call()
assert factoryV2Address == factory_v2_config.get("address"), "factoryV2Address is not correct!"
